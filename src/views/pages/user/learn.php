    ...
    <div class="row m-0 p-0 position-relative">
        <div class="content-main col-md-9 p-0 m-0" style="min-height: 100vh;">
            <div class="w-100 " style="height: 1000px;">
                <iframe style="width: 100%; height: 700px;" src="<?= $current_lession[0]['link_source']; ?>" allowfullscreen></iframe>
        ...
        <div class="content-extra col-md-3 p-0 m-0 position-fixed end-0" style="overflow-y: auto; max-height: 100vh;" id="contentExtra">
            ...
            <div class="list-group">
                <?php
                $i = 1;
                foreach ($data['lessons'] as $lesson) : extract($lesson) ?>
                    <a href="/course/<?= $current_lession[0]['id_course'] ?>/learn/lesson-<?= $num_lesson ?>" class="list-group-item list-group-item-action m-0 border-none py-3">
                        <?php echo $i . ".&emsp;" . $lesson_title ?>
                    </a>
                <?php $i++;
                endforeach; ?>
            </div>
...
<div class="fixed-bottom-container shadow py-2">
    <div class="m-0 p-0 d-flex justify-content-center">
        <div class="button">
            ...
            <a href="/course/<?= $current_lession[0]['id_course'] ?>/learn/lesson-<?= $current_lession[0]['num_lesson'] - 1 ?>" class="btn mx-3 btn-outline-secondary <?= $current_lession[0]['num_lesson'] <= 1 ? "disabled" : ""; ?>" style="width: 100px;">Previous</a>
        </div>

        <div class="button">
            <a href="/course/<?= $current_lession[0]['id_course'] ?>/learn/lesson-<?= $current_lession[0]['num_lesson'] + 1 ?>" class="btn mx-3 btn-outline-primary <?= $current_lession[0]['num_lesson'] >= count($data['lessons']) ? "disabled" : ""; ?>" style="width: 100px;">Next</a>
